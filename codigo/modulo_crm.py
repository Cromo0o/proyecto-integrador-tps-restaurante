...
class ModuloCRM:
    ...
    def evaluar_riesgo_desercion(self):
        ...
        try:
            ...
            # Agrupamos todo el historial de compras por cada cliente individual
            rfm = df.groupby('Customer Name').agg({'Order Time': lambda x: (current_date - x.max()).days}).rename(columns={'Order Time': 'Recency'})
